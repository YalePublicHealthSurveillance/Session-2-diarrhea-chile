...
```{r}
d1 <- d1 %>%
  mutate( icd10.3digits = substr(d1$diag1,1,3)) #extract 1st 3 digits from IC10 code

d1$icd10.3digits[1:10] #view first 10
...
```{r}
sort(table(d1$icd10.3digits),decreasing=T)
```
...
d1 <- d1 %>%
  mutate(a00_a09  = if_else( icd10.3digits %in% c('A00', 'A01', 'A02', 'A03', 'A04', 'A05', 'A06', 'A07', 'A08', 'A09'),1,0))
